...
table.on('click', '#delete', function(e) {
    ...
    var ar_id = data.ar_id;

    // Buka modal
    $('#myFormTitle').text("Delete Record");
    $('#ar_id').val(data.ar_id).prop('disabled', true);
    $('#tanggal').val(data.tanggal);
    $('#jenis').val(data.jenis);
    $('#keterangan').val(data.keterangan);
    $('#amount').val(data.amount);
    ...
    // Handle tombol delete di dalam modal
...
if (isset($_REQUEST["flag"])) {
    if ($_REQUEST["flag"] == "show") {
      ...
      try {
          ...
          $body = file_get_contents('php://input');
          $data = json_decode($body, true);
          $sql = "DELETE FROM ar WHERE ar_id=:ar_id;";
          deleteRow($con, $sql, array("ar_id" => $data['ar_id']));
          $response["status"] = 1;
          $response["message"] = "Ok";
          $response["data"] = $data;
      } catch (Exception $e) {
