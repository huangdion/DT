    ...
    <body>
        ...
        <script>
            ...
            table.on('click', '#delete', function(e) {
                //ambil data dari baris yang diklik
                var row = table.row(e.target.closest('tr')).data();
                var group_id = row.group_id; // Assuming group_id is the correct property name
...
if (isset($_REQUEST["flag"])) {
    if ($_REQUEST["flag"] == "show") {
        ...
        try {
            ...
            $body = file_get_contents('php://input');
            $data = json_decode($body, true);
            $sql = "DELETE FROM deposit WHERE group_id=:group_id AND room_id=:room_id;";
            deleteRow($con, $sql, array("group_id" => $data['group_id'], "room_id" => $data['room_id']));
            $response["status"] = 1;
            $response["message"] = "Ok";
            $response["data"] = $data;
        } catch (Exception $e) {
